...
# # Try optional vLLM import (avoid crashing on CUDA mismatch)
# try:
#     from vllm import LLM, SamplingParams  # type: ignore
# except Exception:  # ImportError or CUDA load errors
...
#         # kwargs = dict(
#         safe_top_k = self.top_k if (self.top_k is not None and self.top_k > 0) else 0
#         kwargs = dict(
...
#             max_new_tokens, self.top_k, self.top_p, self.temperature
#         )

#     @staticmethod
#     def default() -> "GenerationConfig":
#         return GenerationConfig(200, -1, 1.0, 1.0)


# @dataclass(frozen=True)
# class Response:
#     raw_inputs: torch.Tensor
#     raw_outputs: torch.Tensor | None
...
#         # vLLM branch
#         if LLM is not None and isinstance(self.model, LLM):  # type: ignore[arg-type]
#             tokenizer = self.tokenization_context.tokenizer
#             stop_token_ids = [tokenizer.eos_token_id]
#             if "llama-3" in tokenizer.name_or_path:
#                 stop_token_ids.append(tokenizer("<|eot_id|>").input_ids[-1])
...
#             prompt_token_ids = [
#                 input_ids[j, attention_mask[j].bool()].tolist()
#                 for j in range(len(input_ids))
#             ]
#             output_results = self.model.generate(  # type: ignore[attr-defined]
#                 prompt_token_ids=prompt_token_ids, sampling_params=sampling_params
#             )
#             outputs = [
#                 output_results[j].outputs[0].text.replace("<|eot_id|>", "")
...
#         )
#         if LLM is not None and isinstance(self.model, LLM):  # type: ignore[arg-type]
#             output_strings = self.generate(config, input_ids)  # list[str]
#             output_ids = None
#         else:
#             input_ids = input_ids.to(self.model.device)  # type: ignore[attr-defined]
#             output_ids = self.generate(config, input_ids)  # torch.Tensor
#             decoding_config = DecodingConfig(skip_special_tokens=True)
#             output_strings = self.tokenization_context.decode(
#                 decoding_config, output_ids  # type: ignore[arg-type]
#             )
#         return Response(
#             raw_inputs=input_ids,
#             raw_outputs=output_ids if isinstance(output_ids, torch.Tensor) else None,
#             decoded_outputs=output_strings,  # type: ignore[arg-type]
#         )
...
#     DEEPSEEK_CODER_7B = "deepseek-ai/deepseek-coder-7b-base"
#     DEEPSEEK_CODER_33B = "deepseek-ai/deepseek-coder-33b-base"

#     @staticmethod
#     def all() -> list[str]:
#         return [m.value for m in SupportedModelKeys]
...
#             SupportedModelKeys.DEEPSEEK_CODER_7B.value,
...
#         # modern Transformers kwarg
#         other_kwargs["attn_implementation"] = "flash_attention_2"

#     if use_vllm:
#         if LLM is None:
#             raise RuntimeError(
#                 "vLLM is not available (CUDA wheel mismatch). "
#                 "Set USE_VLLM=False or install a CUDA-12 compatible vLLM."
#             )
#         model = LLM(  # type: ignore[call-arg]
#             model_name_or_path,
#             tokenizer=tokenization_context.tokenizer.name_or_path,
...
class ModelContext:
    ...
    def generate(
        ...
        if LLM is not None and isinstance(self.model, LLM):  # type: ignore[arg-type]
            ...
            # vLLM expects -1 to disable top-k (NOT 0)
            vllm_top_k = config.top_k if (config.top_k is not None and config.top_k > 0) else -1
